...
def save_line_svg(rows: list[dict], path: Path, *, value_key: str, title: str) -> None:
    ...
    svg = (
        ...
        + "".join(labels)
        + f'<text x="18" y="{top + plot_height/2:.1f}" font-size="12" transform="rotate(-90 18 {top + plot_height/2:.1f})" text-anchor="middle">AIGC Confidence</text></svg>'
    )
    path.write_text(svg)
...
def write_dashboard(output: Path, payload: dict, *, component_title: str) -> None:
    confidence = float(
        payload["prediction"].get(
            "aigc_confidence", payload["prediction"]["probability_fake"]
        )
    )
    calibrated_probability = float(payload["prediction"]["probability_fake"])
...
<section><h1>单图 AIGC 可解释结果</h1><div class="metric">AIGC Confidence = {confidence:.4f}</div>
<div class="muted">Platt score {calibrated_probability:.4f} · raw logit {raw_logit:+.4f} · {method}</div></section>
...
def write_schema(path: Path) -> None:
    schema = {
        ...
        "properties": {
            ...
            "prediction": {
                "type": "object",
                "required": ["probability_fake", "aigc_confidence", "raw_logit"],
            },
