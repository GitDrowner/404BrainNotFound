...
from .losses import ClassSourceGroupDRO, trace_loss
...
def make_optimizer(model: TraceDetector, config: dict) -> AdamW:
    main, lora = [], []
...
def train(config: dict, max_steps: int | None = None) -> dict:
    ...
    if robust_config.get("enabled", False):
        groups = [
            ClassSourceGroupDRO.group_name(
                int(record["label"]), str(record.get("source", "unknown"))
            )
    ...
    for epoch in range(config["train"]["epochs"]):
            ...
            with autocast_context(device, amp):
                ...
                if group_dro is not None:
                    ...
                    robust_classification, robust_terms = group_dro(
                        per_sample_classification, batch["label"], batch["source"]
                    )
        ...
        selection_metric = config.get("evaluation", {}).get("selection_metric", "auroc")
        score = val_metrics.get(selection_metric, val_metrics["balanced_accuracy"])
