...
def branch_counterfactuals(
    ...
    base_probability = float(torch.sigmoid(model(batch)["logits"].float() / temperature + bias).cpu())
    variants = {}
    components = {
    ...
    for name, keys in components.items():
        ...
        probability = float(torch.sigmoid(model(changed)["logits"].float() / temperature + bias).cpu())
        variants[name] = probability
    return [
        {"component": name, "probability_when_neutralized": probability, "confidence_delta": base_probability - probability}
...
def main() -> None:
    ...
    model = TraceDetector(config, int(config["data"]["image_size"]), int(config["data"]["semantic_image_size"])).to(device)
    ...
    base_rows, base_outputs = score_images(model, [image], config, device, temperature, bias, 1)
    ...
    coarse_scores, _ = score_images(model, occluded, config, device, temperature, bias, args.batch_size)
    patches = []
    for index, (box, score) in enumerate(zip(coarse_boxes, coarse_scores)):
        patches.append({
            ...
            "confidence_contribution": prediction["probability_fake"] - score["probability_fake"],
    ...
    if refined_boxes:
        ...
        refined_scores, _ = score_images(model, refined_images, config, device, temperature, bias, args.batch_size)
    ...
    frequency_total_scores, _ = score_images(
        model, frequency_images, config, device, temperature, bias, args.batch_size
    )
    ...
    transform_scores, _ = score_images(model, transform_images, config, device, temperature, bias, args.batch_size)
    for row, score in zip(transforms, transform_scores):
        row.update(score)

    branches = branch_counterfactuals(model, image, config, device, temperature, bias)
    expert_details = []
    ...
    payload = {
        ...
        "model": {
            "method": "fusion_v2_three_expert" if expert_details else "759921_hybrid_legacy",
    ...
    save_line_svg(transforms, args.output / "transform_trajectory.svg", value_key="probability_fake", title="AIGC confidence under competition transforms")
    component_items = (
