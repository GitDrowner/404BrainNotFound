...
def make_optimizer(
    model: TraceDetector,
    config: dict,
    loss_balancer: UncertaintyWeightedLoss | None = None,
) -> AdamW:
    main, lora = [], []
        ...
        groups.append(
            {
                ...
                "lr": float(config["loss"].get("learning_rate", 0.002)),
                "weight_decay": 0.0,
            }
        )
...
def train(config: dict, max_steps: int | None = None) -> dict:
    ...
    loss_balancer = None
    if config["loss"].get("mode") == "uncertainty":
    ...
    for epoch in range(config["train"]["epochs"]):
        ...
        if loss_balancer is not None:
            checkpoint["uncertainty_weighting"] = {
                "state_dict": {
